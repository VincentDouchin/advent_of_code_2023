...
const part2 = (rawInput: string) => {
  ...
  for (let i =0;i<input.length;i++){
    ...
    const xx = p.map(x=>x.x)
    const max = Math.max(...xx)
    const min = Math.min(...xx)
    for (let j =0;j<=input[i].length;j++){
      const fo= p.find(x=>x.x===j)?.s
      const left = fo && 'LF'.includes(fo)
      const right = fo&&'7F|'.includes(fo)
      
      if(right) ins = !ins
      const pi = p.filter(x=>x.x<j && x.s == '|').length%2==1
      if(!fo &&ins) t++
